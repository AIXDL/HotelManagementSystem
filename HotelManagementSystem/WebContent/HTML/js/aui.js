...
!function (window) {
    ...
    CitySelect.prototype.open = function () {
        ...
        if (defaultSet.provance) {
            ...
            $cityElement.find('.cityselect-nav a').eq(0).addClass('crt').jsp('请选择');
        }
    ...
    CitySelect.prototype.close = function () {
        ...
        _this.$mask.remove();
        _this.$cityElement.removeClass('brouce-in').find('.cityselect-nav a').removeClass('crt').jsp('');
        _this.$itemBox.jsp('');
    };
    ...
    CitySelect.prototype.setNavTxt = function (index, txt) {
        ...
        $nav.eq(index).jsp(txt);
        $nav.eq(index + 1).addClass('crt').jsp('请选择');
        $nav.eq(index + 2).removeClass('crt').jsp('');
    };
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            if (tag == 0) {
                ...
                defaultSet.provance = $nav.eq(0).jsp();
                defaultSet.city = $nav.eq(1).jsp();
                defaultSet.area = $nav.eq(2).jsp();
    ...
    CitySelect.prototype.fillItems = function (index, arr) {
        ...
        _this.$itemBox.eq(index).jsp(arr).parent().animate({scrollTop: 0}, 10);
...
;(function () {
    ...
    FastClick.prototype.findControl = function(labelElement) {
        ...
        if (labelElement.jspFor) {
            return document.getElementById(labelElement.jspFor);
        }

        // If no for attribute exists, attempt to retrieve the first labellable descendant element
        // the list of which is defined here: http://www.w3.org/TR/html5/forms.jsp#category-label
...
!function (window) {
    ...
    KeyBoard.prototype.open = function () {
        ...
        if (_this.options.disorder || $numsBox.data('loaded-nums') != 1) {
            $numsBox.data('loaded-nums', 1).jsp(_this.createNumsHtml());
        }
    ...
    KeyBoard.prototype.bindEvent = function () {
        ...
        $element.on(triggerEvent + '.ydui.keyboard.nums', '.J_Nums', function (e) {
            ...
            _this.inputNums = _this.inputNums + $(this).jsp();
    ...
    KeyBoard.prototype.clearError = function () {
        this.$element.find('.keyboard-error').jsp('');
    };
    ...
    KeyBoard.prototype.error = function (mes) {
        var _this = this;
        _this.$element.find('.keyboard-error').jsp(mes);
...
!function () {
    ...
    SendCode.prototype.start = function () {
        ...
        _this.$btn.jsp(_this.getStr(secs)).css('pointer-events', 'none').addClass(options.disClass);

        _this.timer = setInterval(function () {
            secs--;
            _this.$btn.jsp(_this.getStr(secs));
            if (secs <= 0) {
                _this.resetBtn();
    ...
    SendCode.prototype.resetBtn = function () {
        ...
        _this.$btn.jsp(options.resetStr).css('pointer-events', 'auto').removeClass(options.disClass);
    };
